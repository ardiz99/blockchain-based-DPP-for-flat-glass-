...
if (!productId) { console.error('Usage: node client-dpp/core/getHistory.mjs <productId>'); process.exit(1); }
const { gateway, client, contract } = await newGatewayAndContract();
try {
  const res = await contract.evaluateTransaction('DppCore:GetHistory', productId);
  printResponse(res);
